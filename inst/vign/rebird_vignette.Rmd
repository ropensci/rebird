...
```{r ebirdgeo2}
out <- ebirdgeo(lat = 42, lng = -76, max = 10, includeProvisional = TRUE, hotspot = TRUE)
head(out)
nrow(out)
```
...
```{r ebirdloc1}
out <- ebirdloc(locID = c('L99381','L99382'))
head(out)
```
...
```{r ebirdloc1}
(out <- ebirdloc(locID = 'L99381', species = 'larus delawarensis', max = 10, provisional = TRUE, hotspot=TRUE))
```
...
```{r ebirdloc1}
out <- ebirdregion(region = 'US', species = 'Setophaga caerulescens')
head(out)
```
...
```{r ebirdloc1}
(out <- ebirdregion(region = 'US-OH', max = 10, provisional = TRUE, hotspot = TRUE))
```
...
```{r ebirdloc1}
out <- ebirdnotable(lat = 42, lng = -70)
head(out)
```
